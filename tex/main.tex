...
an open-source Python library; we describe it below.

\textbf{The \texttt{backstab} library.} We release \texttt{backstab} as
an open-source Python library accompanying this paper. It provides exact
rational arithmetic for all three recurrences
(Propositions~\ref{prop:migdal}--\ref{prop:vlopt}) and a Monte Carlo
engine for simulating \(\mathrm{TG}(n, m)\) under any combination of
...
The main result of this paper (Theorem~\ref{thm:pbe}) characterises
equilibrium play through the Vote-Left protocol with punishment. The
propositions below are supporting technical results: exact recurrence
...
target \(T^* \in F\) (in particular, RV+C), Migda\l{}-style recurrences for 
the Traitor win probability require the probability that the day-phase banishment 
lands on a Faithful.
...
F=\{0,\dots,n-m-1\},
\]
and suppose all Traitors collude by voting for the same Faithful target
\(T^\ast = 0\).
...
eliminated player is Faithful is
\[
p_F^{\mathrm{coll,RV}}(n,m)
=
\frac1{|\Omega|}
\sum_{v\in\Omega}
\frac{|W(v)\cap F|}{|W(v)|}.
...
\begin{proof}
Under RV+C, every admissible Faithful
voting profile \(v\in\Omega\) occurs with equal probability. Since each
Faithful player may vote for any player except themselves, the sample
space is precisely \(\Omega\), and hence
\[
...
Because ties are broken uniformly at random, conditional on the profile
\(v\), the probability that the eliminated player is Faithful equals
\[
\frac{|W(v)\cap F|}{|W(v)|}.
\]

Taking the expectation over all admissible voting profiles gives
\[
P_{\mathrm{faithful}}^{\mathrm{RV+C}}(n,m)
=
\sum_{v\in\Omega}
\mathbb{P}(v)
\frac{|W(v)\cap F|}{|W(v)|}.
...
Since \(\mathbb{P}(v)=1/|\Omega|\) for all \(v\in\Omega\),
\[
p_F^{\mathrm{coll,RV}}(n,m)
=
\frac1{|\Omega|}
...
\subsection{Recurrences for RV+C and VL+Opt}\label{sec:recurrences}

Lemma~\ref{lemma:collusion} replaces the uniform banishment probability
\((n - m)/n\) of the Migda\l{} recurrence
(Proposition~\ref{prop:migdal}) with a strategy-specific banishment
probability. Adapting the recurrence accordingly characterises the
Traitor win probability under RV+C and under \(\sigma^\dagger\) exactly.

\begin{figure}[htbp!]
\centering
\includegraphics[width=\textwidth]{fig_strategy_progression.pdf}
\caption{\textbf{Strategic game progression.} \textbf{Top row:} voting
patterns under RV (random, uncoordinated), RV+C (random with Traitor
collusion), and VL (cyclic Vote-Left). \textbf{Bottom row:} the Traitors'
...
numbers to punish, so Traitors collude. Arrows show votes
(F = Faithful, blue; T = Traitor, red).}
\label{fig:strategy_progression}
\end{figure}

\begin{proposition}[Traitor win probability under RV+C]\label{prop:rvc}
Let \(w_{\ddagger}(n,m)\) denote the probability that at least one Traitor is
among the (possibly tied) winners in a population of \(n\) players with
\(m\) Traitors, under random voting with collusion (RV+C).
...
0 & m = 0, \\[6pt]
1 & n \leq 2m, \\[6pt]
p_F^{\mathrm{coll,RV}}(n,m)\, w_{\ddagger}(n-2,m)
+ \bigl(1 - p_F^{\mathrm{coll,RV}}(n,m)\bigr)\, w_{\ddagger}(n-2,m-1)
& n > 2m.
\end{cases}
\]
...
Under RV+C the Traitors collude every round, so the day-phase banishment is a
Faithful with probability \(p_F^{\mathrm{coll, RV}}\) and a Traitor otherwise;
the night phase always removes one further Faithful. The recurrence then follows
from the law of total probability, exactly as in Proposition~\ref{prop:migdal}.
\end{proof}
...
\end{cases}
\]
The recurrence differs from the Migda\l{} recurrence
(Proposition~\ref{prop:migdal}) only in the late-game boundary: states
with \(2m < n \leq 2m + 2\), which Migda\l{} continues to break down by
weighted average, are absorbed by \(\sigma^\dagger\) into the
Traitor-win region \(w_\dagger = 1\) because optimal collusion forces
the Faithful banishment.
\end{proposition}
...
with Collusion (RV+C): Traitors collude on a single Faithful target
each round while Faithful vote at random; the Faithful win rate
collapses sharply. \textbf{C,} Vote-Left with Collusion and detection
...
Figure~\ref{fig:strategy}A--B shows that VL and RV produce identical
compliance outcomes, matching the exact Migda\l{} value, while RV+C
collapses the Faithful win rate. Figure~\ref{fig:strategy}C confirms
that detection and punishment under VL+C substantially recovers the
Faithful win rate relative to RV+C. Figure~\ref{fig:strategy}D--E
shows that VL+Comp recovers the compliance baseline, and that
\(\sigma^\dagger\) (VL+Opt) erodes it only modestly in the late-game
phase. Figure~\ref{fig:strategy_ratios} quantifies how much the Traitor
Nash equilibrium strategy \(\sigma^\dagger\) (VL+Opt) improves the
Traitor position relative to each alternative.
...
\textbf{C,} VL+Opt / VL+C: below one, confirming that Traitors prefer
